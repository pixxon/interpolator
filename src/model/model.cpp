...
namespace Model
{
    Model::Model():
        QObject()
    {
        _table = Parseval::SymbolTable::getInstance();

        _table->insertSymbol("var", QRegExp("^(x|y)"), Parseval::ARITY_CONSTANT, Parseval::ASSOCIATIVITY_NONE, 0, 0);
        _table->insertSymbol("num", QRegExp("^[0-9]+(\\.[0-9]+)?"), Parseval::ARITY_CONSTANT, Parseval::ASSOCIATIVITY_NONE, 0, 0);
        _table->insertSymbol("open", QRegExp("^\\("), Parseval::ARITY_CONSTANT, Parseval::ASSOCIATIVITY_NONE, 0, 0);
        _table->insertSymbol("close", QRegExp("^\\)"), Parseval::ARITY_CONSTANT, Parseval::ASSOCIATIVITY_NONE, 0, 0);

        _table->insertSymbol("add", QRegExp("^\\+"), Parseval::ARITY_BINARY, Parseval::ASSOCIATIVITY_LEFT, 1, [](double a, double b){ return a + b; });
        _table->insertSymbol("min", QRegExp("^-"), Parseval::ARITY_BINARY, Parseval::ASSOCIATIVITY_LEFT, 1, [](double a, double b){ return a - b; });
        _table->insertSymbol("mul", QRegExp("^\\*"), Parseval::ARITY_BINARY, Parseval::ASSOCIATIVITY_LEFT, 2, [](double a, double b){ return a * b; });
        _table->insertSymbol("div", QRegExp("^/"), Parseval::ARITY_BINARY, Parseval::ASSOCIATIVITY_LEFT, 2, [](double a, double b){ return a / b; });
        _table->insertSymbol("pow", QRegExp("^\\^"), Parseval::ARITY_BINARY, Parseval::ASSOCIATIVITY_RIGHT, 3, [](double a, double b){ return pow(a, b); });

        _table->insertSymbol("abs", QRegExp("^abs"), Parseval::ARITY_UNARY, Parseval::ASSOCIATIVITY_NONE, 0, [](double a, double){ return a<0?-a:a; });

        _table->insertSymbol("sin", QRegExp("^sin"), Parseval::ARITY_UNARY, Parseval::ASSOCIATIVITY_NONE, 0, [](double a, double){ return sin(a); });
        _table->insertSymbol("cos", QRegExp("^cos"), Parseval::ARITY_UNARY, Parseval::ASSOCIATIVITY_NONE, 0, [](double a, double){ return cos(a); });
        _table->insertSymbol("tg", QRegExp("^tg"), Parseval::ARITY_UNARY, Parseval::ASSOCIATIVITY_NONE, 0, [](double a, double){ return tan(a); });
        _table->insertSymbol("ctg", QRegExp("^ctg"), Parseval::ARITY_UNARY, Parseval::ASSOCIATIVITY_NONE, 0, [](double a, double){ return 1 / tan(a); });

        _evaluator = new Parseval::Evaluator();
        ...
        _interpolator = new Interpolate::Lagrange();
    ...
    void Model::setPart(char var, double min, double max, int count, QString type)
    {
        if (var == 'x')
        {
            if (type == "Egyenletes")
            {
                _partX.setPartition(min, max, count, Interpolate::PARTITION_TYPE_EVEN);
            }
            if (type == "Csebisev")
            {
                _partX.setPartition(min, max, count, Interpolate::PARTITION_TYPE_CHEBYSHEV);
            }
        }
        if (var == 'y')
        {
            if (type == "Egyenletes")
            {
                _partY.setPartition(min, max, count, Interpolate::PARTITION_TYPE_EVEN);
            }
            if (type == "Csebisev")
            {
                _partY.setPartition(min, max, count, Interpolate::PARTITION_TYPE_CHEBYSHEV);
            }
        }
    ...
    void Model::setTwoDimension(bool enabled)
    {
        if (enabled)
        {
            ...
            _table->insertSymbol("var", QRegExp("^x"), Parseval::ARITY_CONSTANT, Parseval::ASSOCIATIVITY_NONE, 0, 0);
            delete _interpolator;
            _interpolator = new Interpolate::Newton();
        }
        else
        {
            _table->removeSymbol("var");
